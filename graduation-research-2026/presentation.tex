  ...
  \begin{columns}
    \begin{column}{0.58\textwidth}
      ...
      \begin{itemize}
        ...
        \item 長さ $\Delta l = l / N$ の剛体棒と質量 $\Delta m = m / N$ の質点からなる$N$重振り子として定式化．
    ...
    \begin{column}{0.4\textwidth}
      ...
      \draw[thin] (O) -- +(30:1.0);
      \draw[thin] (P4) -- +(30:1.0);
      \draw[<->, thick] (30:0.8) -- ++(\ang:4.2) node[midway, above right] {$l$};
    \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{解析手法：アプローチ}
  ...
  \begin{enumerate}
    ...
    \item $N \rightarrow \infty$ の極限を取り，連続体としての挙動を解析．
  \end{enumerate}
\end{frame}
...
\begin{frame}{結果：運動方程式の導出}
  $\alpha_k$を得るために，ラグランジュ方程式に$t = 0$を代入した式
  \begin{equation}
  ...
  (10)を代入して整理すると，$\alpha_j$に関する次の連立方程式が得られる：
  \begin{equation}
...
\begin{frame}{結果：$N \rightarrow \infty$ の極限と積分方程式}
  ...
  \begin{equation}
    \int_{0}^{l} \left(l - \max(s', s)\right) \alpha(s') \,\mathrm{d}s' + g(l - s)\sin\theta = 0
  \end{equation}
  積分区間を分割して整理すると：
  \begin{equation}
  ...
  両辺を $s$ について微分して整理すると：
  \begin{equation}
    \int_{0}^{s} \alpha(s') \,\mathrm{d}s' = -g \sin\theta
  \end{equation}
\end{frame}
...
\begin{frame}{結論と今後の展望}
  ...
  \begin{itemize}
    ...
    \item 固定端にはデルタ関数的な特異性が現れ，数学的結果と物理的現実との間に矛盾が生じた（現実には角度が無限大になることはない）．
  \end{itemize}
  \vspace{1em}

  \textbf{今後の展望}
  \begin{itemize}
    \item 現実の紐が持つ「\textbf{太さ}」や「\textbf{曲げ剛性}」を考慮に入れる．
    \item $t$ の3次以上の項を無視した微小時間だけでなく，有限な時間経過における落下挙動（折れ曲がりの伝搬など）を解析する手法の構築．
  \end{itemize}
\end{frame}
