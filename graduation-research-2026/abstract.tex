...
\begin{enumerate}
  ...
  \item 紐の長さを$l$，質量を$m$とし，これを$N$等分した．図\ref{fig:n_pendulum}に示すように，紐を
  ...
  \item 落下挙動を調べるため，時刻$t$は微少量であると仮定した．時刻$t = 0$における各質点の角加速度を$\alpha_k$とおき，
        各質点の鉛直線となす角度$\varphi_k$はテイラー展開を用いて$\varphi_k = \theta + \frac{1}{2} \alpha_k t^2$と近似した．
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.2, >=stealth]
    ...
    \draw[thin] (O) -- +(30:0.8);
    \draw[thin] (P4) -- +(30:0.8);
    \draw[<->, thick] (30:0.6) -- ++(\ang:4.2) node[midway, above right] {$l$};
  \end{tikzpicture}
  \caption{}
...
これにより，初角加速度は$s'$の関数$\alpha \left(s'\right)$となり，和は積分に置き換わるため，
...
結果として，固定端からの長さが$s$である点における，初角加速度$\alpha \left(s\right)$及び時刻$t$における角度$\varphi \left(s\right)$は以下のように求められた．
\begin{equation}
\begin{aligned}
...
\section*{補足資料：詳細な計算過程}
\noindent
\begin{minipage}[c]{0.80\columnwidth}
  \scriptsize\url{https://github.com/yuichiro-kurose/physics/blob/master/graduation-research-2026/supplementary-material.pdf}
\end{minipage}%
\hfill
